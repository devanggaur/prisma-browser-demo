...
interface BrowserFrameProps {
  ...
  onNavigate?: (url: string, addTab?: (tab: TabData) => void, fromTab?: string) => void;
  children: React.ReactNode;
}

const defaultTabs: TabData[] = [
  { id: "newtab", label: "New Tab", url: "about:blank", closeable: false },
];
...
export function BrowserFrame({
  ...
  const addTabDynamic = useCallback((tab: TabData) => {
    setTabs(prevTabs => {
  ...
  const handleAddressSubmit = (e: React.FormEvent) => {
    ...
    if (onNavigate) {
      onNavigate(addressInput, addTabDynamic, currentTab);
    }
  };
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-1 px-2 pt-3">
          {tabs.map((tab) => (
            ...
            >
              <span className="truncate max-w-[350px]">{tab.label}</span>
              {tab.closeable && (
                <X
